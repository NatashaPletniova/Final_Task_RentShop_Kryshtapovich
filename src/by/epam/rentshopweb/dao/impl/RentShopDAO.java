...
public class RentShopDAO implements RentShop {
	...
	private final static String PRICE_PER_DAY = "PricePerDay";

	private final static String SELECT_PARTICULAR_ITEM = "SELECT pi.ProductItemID, ProductName, ProductBasePrice, ManufacturingYear, WearRate, "
			+ " CAST((ProductBasePrice - (ProductBasePrice * WearRate)* 0.01) AS  money) AS PricePerDay"
			+ " FROM ProductItem pi JOIN Product p ON pi.ProductID = p.ProductID WHERE  pi.ProductItemID = ?;";
	...
	private final static String SELECT_AVAILABLE_FROM_PRODUCT = "SELECT ProductID," + "ProductCategoryID,"
			+ "ProductName," + "ProductBasePrice," + "Quantity," + "AvailableQuantity "
			+ "FROM Product WHERE AvailableQuantity > 0;";
	...
	public void addProductItem(int productID, int manufacturingYear, String itemNote, int status, int wearRate,
		...
		try {
			...
			if (productBasePrice == null) {
				...
				cs.setDouble(8, productBasePrice);
			}
			cs.execute();
	...
	@Override
	public int rentProductItem(int productItemID, int rentPeriod, String clientName) throws DaoException {
	...
	@Override
	public List<Product> showAllProduct() throws DaoException {
		...
		try {
			ps = con.prepareStatement(SELECT_AVAILABLE_FROM_PRODUCT);
			rs = ps.executeQuery();
	...
	@Override
	public ProductItem showParticularProductItem(int productItemID) throws DaoException {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		ConnectionPool connectionPool = ConnectionPool.getInstance();
		...
		try {
			ps = con.prepareStatement(SELECT_PARTICULAR_ITEM);
			ps.setInt(1, productItemID);
			...
			while (rs.next()) {
				...
				productItem.setPricePerDay(rs.getBigDecimal(PRICE_PER_DAY));
			}
